...
const lanes = [
  ...
  },
  {
    ...
    checks: [
      {
        id: "review_dates_ready",
        label: "review dates ready",
        args: ["--require-ready"],
      },
    ],
  },
...
function inspectLane(lane) {
  ...
  const checks = lane.checks.map((check) => ({
    ...
    label: check.label,
    passed: false,
    command: validatorCommand(lane, lane.localFile, check.args),
    args: check.args,
  }));
  ...
  const status = passed === lane.checks.length
    ? "ready"
    : passed > 0
      ? "partial"
      : "local_not_ready";
...
function printText(status) {
  ...
  for (const lane of status.lanes) {
    console.log(`- ${lane.id}: ${lane.status} (${lane.localFile})`);
